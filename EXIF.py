...
class Exif(dict):
    ...
    def to_bytes(self):
        ...
        zeroth_set = self._dict_to_bytes(zeroth_ifd, "Zeroth", 0)
        zeroth_length = (len(zeroth_set[0]) + exif_is * 12 + gps_is * 12 + 4 +
                         len(zeroth_set[1]))

        if exif_is:
            exif_set = self._dict_to_bytes(exif_ifd, "Exif", zeroth_length)
    ...
    def _split_ifd(self):
        ...
        zeroth_ifd = dict((key, exif_dict[key])
                          for key in exif_dict if key in TAGS["Zeroth"])
        exif_ifd = dict((key, exif_dict[key])
                        for key in exif_dict if key in TAGS["Exif"])
