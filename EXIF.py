...
class _ExifReader(object):
    ...
    def get_exif(self):
        ...
        if EXIF_POINTER in zeroth_dict:
            pointer = struct.unpack(self.endian_mark + "L", zeroth_dict[EXIF_POINTER][2])[0]
            exif_dict = self.get_ifd_dict(pointer)

        if GPS_POINTER in zeroth_dict:
            pointer = struct.unpack(self.endian_mark + "L", zeroth_dict[GPS_POINTER][2])[0]
...
class Exif(dict):
    ...
    def to_bytes(self):
        ...
        if exif_is:
            ...
            exif_length = 0
        if gps_is:
            gps_set = self._dict_to_bytes(gps_ifd, "GPSInfo", zeroth_length + exif_length)
        ...
        if exif_is:
            ...
            exif_pointer = b""
        if gps_is:
            pointer_value = self.TIFF_HEADER_LENGTH + zeroth_length + exif_length
            pointer_str = struct.pack(">I", pointer_value)
            key = 34853
            key_str = struct.pack(">H", key)
            type_str = struct.pack(">H", TYPES["Long"])
    ...
    def _dict_to_bytes(self, ifd_dict, name, ifd_offset):
        ...
        for n, key in enumerate(sorted(ifd_dict)):
            ...
            if value_type == "Byte":
                ...
                if length > 4:
                    offset = self.TIFF_HEADER_LENGTH + ifd_offset + entries_length + len(values)
